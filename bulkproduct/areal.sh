...
file=$(ls *.txt)
path="/home/manish/puppeteer-scraper/flipkart_urls/bulkproduct"
...
if [ -z "$current_price" ]; then
    echo "Price not found for $file on $record_date" |tee -a  $path/infologs.txt
else
    # Compare with last recorded price
    if [ "$last_price" -eq "$current_price" ]; then
        echo "Status is the same for $file price @ $current_price $record_date" |tee -a  $path/infologs.txt
    else
        current_date=$(date "+%Y-%m-%d %H:%M:%S")
        c_date=$(date "+%Y-%m-%d_%H:%M:%S")

        # Log to main history file
        echo "$file, $c_date, $current_price" |tee -a  $path/allpricehistory.txt
        ...
        echo "Price changed! $current_date $last_price to $current_price for $file $url and lowest was $lowest_price, here is list $list" \
            |tee -a  $path/listofchangedprice.txt
    fi
fi
