...
db_path="$script_dir/../limited_product_prices.db"
log_base="$script_dir/../"
...
sqlite3 "$db_path" <<EOF
CREATE TABLE IF NOT EXISTS limited_product_price_history (
...
if [ -z "$current_price" ]; then
    ...
    if ! grep -qE '^[0-9]+$' "$file" 2>/dev/null; then
        ...
        sqlite3 "$db_path" <<EOF
INSERT INTO limited_product_price_history (product_file, record_date, price, rating, url)
...
EOF
            ...
            # Email notification
            echo "rating is $ratingmail as on $record_date and ip was `curl ifconfig.io` Price changed! $last_price to $current_price for $file $url and lowest was $lowest_price, here is list $list" \
